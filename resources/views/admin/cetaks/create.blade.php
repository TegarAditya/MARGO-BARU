        ...
        $('#product-search').on('select2:select', function(e) {
            ...
            $.ajax({
                ...
                success: function(product) {
                                    ...
                                    <div class="col" style="max-width: 160px">
                                        ...
                                        <div class="form-group text-field m-0">
                                            <div class="text-field-input px-2 py-0">
                                                ...
                                                <input class="form-control text-center quantity_text" type="text" name="quantity_text[]" value="" required>
                                                <label class="text-field-border"></label>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    // change 1: Initialize Select2 with AJAX
                    $('.plates').select2({
                        ajax: {
                            ...
                            dataType: 'json',
                            processResults: function(data) {
                                // change 1.1: Process the data from the AJAX response
                                return {
                                    results: data
                                };
                            }
                    ...
                    // change 2: Function to load data and set default
                    function loadDefaultPlate() {
                        $.ajax({
                            url: "{{ route('admin.materials.getPlateRaws') }}",
                            data: {
                                vendor: $('#vendor_id').val(),
                                product: $('.plates').data('product')
                            },
                            dataType: 'json',
                            success: function(data) {
                                if (data.length > 0) {
                                    // Add the first item as the default selection
                                    $('.plates').append(new Option(data[0].text, data[0].id, true, true)).trigger('change');
                                } else {
                                    Swal.fire({
                    ...
                    // change 3: Call the function to load the data and set the default option when the DOM is ready
                    $(document).ready(function() {
                        loadDefaultPlate();
                    });
                    ...
                    productItem.each(function(index, item) {
                        ...
                        quantityText.on('input change', function(e) {
                            ...
                            if (!e.target.value.trim()) {
                                ...
                                quantity.val(valueNum).trigger('change');
                            }
                        }).trigger('change');
