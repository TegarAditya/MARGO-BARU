...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.stockSaldo.title_singular') }}
    </div>

    <div class="card-body">
        <form class="form-prevent-multiple-submits" method="POST" action="{{ route("admin.stock-saldos.update", [$stockSaldo->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('product') ? 'is-invalid' : '' }}" name="product_id" id="product_id">
                    @foreach($products as $id => $entry)
                        <option value="{{ $id }}" {{ (old('product_id') ? old('product_id') : $stockSaldo->product->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('material') ? 'is-invalid' : '' }}" name="material_id" id="material_id">
                    @foreach($materials as $id => $entry)
                        <option value="{{ $id }}" {{ (old('material_id') ? old('material_id') : $stockSaldo->material->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('periode') ? 'is-invalid' : '' }}" type="text" name="periode" id="periode" value="{{ old('periode', $stockSaldo->periode) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('start_date') ? 'is-invalid' : '' }}" type="text" name="start_date" id="start_date" value="{{ old('start_date', $stockSaldo->start_date) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('end_date') ? 'is-invalid' : '' }}" type="text" name="end_date" id="end_date" value="{{ old('end_date', $stockSaldo->end_date) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('qty_awal') ? 'is-invalid' : '' }}" type="number" name="qty_awal" id="qty_awal" value="{{ old('qty_awal', $stockSaldo->qty_awal) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('in') ? 'is-invalid' : '' }}" type="number" name="in" id="in" value="{{ old('in', $stockSaldo->in) }}" step="0.01" required>
                @if($errors->has('in'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('out') ? 'is-invalid' : '' }}" type="number" name="out" id="out" value="{{ old('out', $stockSaldo->out) }}" step="0.01" required>
                @if($errors->has('out'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('qty_akhir') ? 'is-invalid' : '' }}" type="number" name="qty_akhir" id="qty_akhir" value="{{ old('qty_akhir', $stockSaldo->qty_akhir) }}" step="0.01" required>
