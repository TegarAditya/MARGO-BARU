...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.groupArea.title_singular') }}
    </div>

    <div class="card-body">
        <form class="form-prevent-multiple-submits" method="POST" action="{{ route("admin.group-areas.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('provinsi') ? 'is-invalid' : '' }}" name="provinsi" id="provinsi" required>
                    ...
                    @foreach(App\Models\GroupArea::PROVINSI_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('provinsi', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
