...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.finishingMasuk.title_singular') }}
    </div>

    <div class="card-body">
        <form class="form-prevent-multiple-submits" method="POST" action="{{ route("admin.finishing-masuks.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('finishing_item') ? 'is-invalid' : '' }}" name="finishing_item_id" id="finishing_item_id">
                    @foreach($finishing_items as $id => $entry)
                        <option value="{{ $id }}" {{ old('finishing_item_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
