...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.estimationMovement.title_singular') }}
    </div>

    <div class="card-body">
        <form class="form-prevent-multiple-submits" method="POST" action="{{ route("admin.estimation-movements.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="movement_date">{{ trans('cruds.estimationMovement.fields.movement_date') }}</label>
                <input class="form-control date {{ $errors->has('movement_date') ? 'is-invalid' : '' }}" type="text" name="movement_date" id="movement_date" value="{{ old('movement_date') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('movement_type') ? 'is-invalid' : '' }}" name="movement_type" id="movement_type" required>
                    ...
                    @foreach(App\Models\EstimationMovement::MOVEMENT_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('movement_type', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
