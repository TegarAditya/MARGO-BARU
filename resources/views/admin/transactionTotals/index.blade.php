@extends('layouts.admin')
@section('content')

<div class="card">
    <div class="card-header">
        {{ trans('cruds.transactionTotal.title_singular') }} {{ trans('global.list') }}
    </div>

    <div class="card-body">
        <table class=" table table-bordered table-striped table-hover ajaxTable datatable datatable-TransactionTotal">
            <thead>
                <tr>
                    <th width="10">

                    </th>
                    <th>
                        {{ trans('cruds.transactionTotal.fields.salesperson') }}
                    </th>
                    <th>
                        {{ trans('cruds.transactionTotal.fields.total_invoice') }}
                    </th>
                    <th>
                        {{ trans('cruds.transactionTotal.fields.total_diskon') }}
                    </th>
                    <th>
                        {{ trans('cruds.transactionTotal.fields.total_retur') }}
                    </th>
                    <th>
                        {{ trans('cruds.transactionTotal.fields.total_bayar') }}
                    </th>
                    <th>
                        {{ trans('cruds.transactionTotal.fields.total_potongan') }}
                    </th>
                    <th>
                        &nbsp;
                    </th>
                </tr>
            </thead>
        </table>
    </div>
</div>



@endsection
@section('scripts')
@parent
<script>
    $(function () {
  let dtButtons = $.extend(true, [], $.fn.dataTable.defaults.buttons)
  
  let dtOverrideGlobals = {
    buttons: dtButtons,
    processing: true,
    serverSide: true,
    retrieve: true,
    aaSorting: [],
    ajax: "{{ route('admin.transaction-totals.index') }}",
    columns: [
      { data: 'placeholder', name: 'placeholder' },
{ data: 'salesperson_code', name: 'salesperson.code' },
{ data: 'total_invoice', name: 'total_invoice' },
{ data: 'total_diskon', name: 'total_diskon' },
{ data: 'total_retur', name: 'total_retur' },
{ data: 'total_bayar', name: 'total_bayar' },
{ data: 'total_potongan', name: 'total_potongan' },
{ data: 'actions', name: '{{ trans('global.actions') }}' }
    ],
    orderCellsTop: true,
    order: [[ 1, 'desc' ]],
    pageLength: 50,
  };
  let table = $('.datatable-TransactionTotal').DataTable(dtOverrideGlobals);
  $('a[data-toggle="tab"]').on('shown.bs.tab click', function(e){
      $($.fn.dataTable.tables(true)).DataTable()
          .columns.adjust();
  });
  
});

</script>
@endsection