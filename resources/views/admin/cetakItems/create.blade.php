...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.cetakItem.title_singular') }}
    </div>

    <div class="card-body">
        <form class="form-prevent-multiple-submits" method="POST" action="{{ route("admin.cetak-items.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('plate') ? 'is-invalid' : '' }}" name="plate_id" id="plate_id">
                    @foreach($plates as $id => $entry)
                        <option value="{{ $id }}" {{ old('plate_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('plate'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('plate_cost') ? 'is-invalid' : '' }}" type="number" name="plate_cost" id="plate_cost" value="{{ old('plate_cost', '') }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('paper_cost') ? 'is-invalid' : '' }}" type="number" name="paper_cost" id="paper_cost" value="{{ old('paper_cost', '') }}" step="0.01">
