...
<div class="card">
    ...
    <div class="card-body">
        <form class="form-prevent-multiple-submits" method="POST" action="{{ route("admin.sales-reports.update", [$salesReport->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="code">{{ trans('cruds.salesReport.fields.code') }}</label>
                <input class="form-control {{ $errors->has('code') ? 'is-invalid' : '' }}" type="text" name="code" id="code" value="{{ old('code', $salesReport->code) }}" required>
                @if($errors->has('code'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('periode') ? 'is-invalid' : '' }}" type="text" name="periode" id="periode" value="{{ old('periode', $salesReport->periode) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('salesperson') ? 'is-invalid' : '' }}" name="salesperson_id" id="salesperson_id" required>
                    @foreach($salespeople as $id => $entry)
                        <option value="{{ $id }}" {{ (old('salesperson_id') ? old('salesperson_id') : $salesReport->salesperson->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('start_date') ? 'is-invalid' : '' }}" type="text" name="start_date" id="start_date" value="{{ old('start_date', $salesReport->start_date) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('end_date') ? 'is-invalid' : '' }}" type="text" name="end_date" id="end_date" value="{{ old('end_date', $salesReport->end_date) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type">
                    ...
                    @foreach(App\Models\SalesReport::TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('type', $salesReport->type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('saldo_awal') ? 'is-invalid' : '' }}" type="number" name="saldo_awal" id="saldo_awal" value="{{ old('saldo_awal', $salesReport->saldo_awal) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('debet') ? 'is-invalid' : '' }}" type="number" name="debet" id="debet" value="{{ old('debet', $salesReport->debet) }}" step="0.01" required>
                @if($errors->has('debet'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('kredit') ? 'is-invalid' : '' }}" type="number" name="kredit" id="kredit" value="{{ old('kredit', $salesReport->kredit) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('saldo_akhir') ? 'is-invalid' : '' }}" type="number" name="saldo_akhir" id="saldo_akhir" value="{{ old('saldo_akhir', $salesReport->saldo_akhir) }}" step="0.01" required>
