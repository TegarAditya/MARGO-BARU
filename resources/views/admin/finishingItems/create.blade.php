...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.finishingItem.title_singular') }}
    </div>

    <div class="card-body">
        <form class="form-prevent-multiple-submits" method="POST" action="{{ route("admin.finishing-items.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('buku') ? 'is-invalid' : '' }}" name="buku_id" id="buku_id" required>
                    @foreach($bukus as $id => $entry)
                        <option value="{{ $id }}" {{ old('buku_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('buku'))
