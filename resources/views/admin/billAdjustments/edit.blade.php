...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.billAdjustment.title_singular') }}
    </div>
    ...
    <div class="card-body">
        ...
        <form class="form-prevent-multiple-submits" id="paymentForm" method="POST" action="{{ route("admin.bill-adjustments.update", [$billAdjustment->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <input type="hidden" name="amount" value="{{ $billAdjustment->amount }}" />
            <div class="row">
                <div class="col-6">
                    <div class="form-group">
                        <label class="required" for="no_adjustment">{{ trans('cruds.billAdjustment.fields.no_adjustment') }}</label>
                        <input class="form-control {{ $errors->has('no_adjustment') ? 'is-invalid' : '' }}" type="text" name="no_adjustment" id="no_adjustment" value="{{ old('no_adjustment', $no_adjustment) }}" readonly>
                ...
                <div class="col-6">
                    <div class="form-group">
                        ...
                        <input class="form-control date {{ $errors->has('date') ? 'is-invalid' : '' }}" type="text" name="date" id="date" value="{{ old('date', $billAdjustment->date) }}" required>
                        @if($errors->has('date'))
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2 {{ $errors->has('salesperson') ? 'is-invalid' : '' }}" name="salesperson_id" id="salesperson_id" required>
                            @foreach($salespeople as $id => $entry)
                                <option value="{{ $id }}" {{ (old('salesperson_id') ? old('salesperson_id') : $billAdjustment->salesperson->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <div class="form-group text-field m-0">
                            <div class="text-field-input px-2 py-0">
                                ...
                                <input class="form-control" type="text" id="bayar_text" name="bayar_text" min="1" value="{{ angka($billAdjustment->amount) }}">
                ...
                <div class="col-12">
                    <div class="form-group">
                        ...
                        <textarea class="form-control {{ $errors->has('note') ? 'is-invalid' : '' }}" name="note" id="note">{{ old('note', $billAdjustment->note) }}</textarea>
                        @if($errors->has('note'))
