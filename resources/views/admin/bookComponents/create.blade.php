...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.bookComponent.title_singular') }}
    </div>

    <div class="card-body">
        <form class="form-prevent-multiple-submits" method="POST" action="{{ route("admin.book-components.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('components') ? 'is-invalid' : '' }}" name="components[]" id="components" multiple>
                    @foreach($components as $id => $component)
                        <option value="{{ $id }}" {{ in_array($id, old('components', [])) ? 'selected' : '' }}>{{ $component }}</option>
                    @endforeach
                </select>
