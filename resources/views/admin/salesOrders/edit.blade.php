    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.sales-orders.update", [$salesOrder->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('product') ? 'is-invalid' : '' }}" name="product_id" id="product_id" required>
                    @foreach($products as $id => $entry)
                        <option value="{{ $id }}" {{ (old('product_id') ? old('product_id') : $salesOrder->product->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kurikulum') ? 'is-invalid' : '' }}" name="kurikulum_id" id="kurikulum_id">
                    @foreach($kurikulums as $id => $entry)
                        <option value="{{ $id }}" {{ (old('kurikulum_id') ? old('kurikulum_id') : $salesOrder->kurikulum->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('quantity') ? 'is-invalid' : '' }}" type="number" name="quantity" id="quantity" value="{{ old('quantity', $salesOrder->quantity) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('moved') ? 'is-invalid' : '' }}" type="number" name="moved" id="moved" value="{{ old('moved', $salesOrder->moved) }}" step="1">
                @if($errors->has('moved'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('retur') ? 'is-invalid' : '' }}" type="number" name="retur" id="retur" value="{{ old('retur', $salesOrder->retur) }}" step="1">
                @if($errors->has('retur'))
