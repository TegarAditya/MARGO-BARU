...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.stockAdjustmentDetail.title_singular') }}
    </div>

    <div class="card-body">
        <form class="form-prevent-multiple-submits" method="POST" action="{{ route("admin.stock-adjustment-details.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('stock_adjustment') ? 'is-invalid' : '' }}" name="stock_adjustment_id" id="stock_adjustment_id" required>
                    @foreach($stock_adjustments as $id => $entry)
                        <option value="{{ $id }}" {{ old('stock_adjustment_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
